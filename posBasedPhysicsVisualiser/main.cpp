#define Fullscreen true //eigen variabele; bepaalt later in de code of fullscreen of windowed mode gebruikt word
#define GLEW_STATIC
...
int main() {
    ...
    #if false //again, first code is original, second one is from download
   ...
   /*#if false //verify correct configuration of glew:
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        View = glm::lookAt(
            glm::vec3(3 * cos(time), 2 + 2 * sin (time / 2.1), 3 * sin(time)), // Camera is at (4,3,3), in World Space
            glm::vec3(0, 0, 0), // and looks at the origin
            glm::vec3(0, 1, 0)  // Head is up (set to 0,-1,0 to look upside-down)
        );
